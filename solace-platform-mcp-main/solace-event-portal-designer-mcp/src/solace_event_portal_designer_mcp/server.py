...
def main():
    ...
    mcp = FastMCP.from_openapi(
        ...
        route_maps=[
            ...
            RouteMap(pattern=r"^/api/v2/architecture/applicationVersions/\{versionId\}/state$", mcp_type=MCPType.TOOL),
            ...
            RouteMap(pattern=r"^/api/v2/architecture/eventVersions/\{id\}/state$", mcp_type=MCPType.TOOL),
            ...
            RouteMap(pattern=r"^/api/v2/architecture/schemaVersions/\{id\}/state$", mcp_type=MCPType.TOOL),
            # Enums and Enum Versions (needed for schema version checks)
            ...
            RouteMap(pattern=r"^/api/v2/architecture/enumVersions/\{id\}/state$", mcp_type=MCPType.TOOL),
            # Event APIs — list, create, get by id, update, delete
            RouteMap(pattern=r"^/api/v2/architecture/eventApis(/\{id\})?$", mcp_type=MCPType.TOOL),
            # Event API Versions — CRUD + AsyncAPI export + state management
            RouteMap(pattern=r"^/api/v2/architecture/eventApiVersions(/\{versionId\})?$", mcp_type=MCPType.TOOL),
            RouteMap(pattern=r"^/api/v2/architecture/eventApiVersions/\{eventApiVersionId\}/asyncApi$", mcp_type=MCPType.TOOL),
            RouteMap(pattern=r"^/api/v2/architecture/eventApiVersions/\{eventApiVersionId\}/exports/asyncApi$", mcp_type=MCPType.TOOL),
            RouteMap(pattern=r"^/api/v2/architecture/eventApiVersions/\{versionId\}/state$", mcp_type=MCPType.TOOL),
            # Event API Products — list, create, get by id, update, delete
            RouteMap(pattern=r"^/api/v2/architecture/eventApiProducts(/\{id\})?$", mcp_type=MCPType.TOOL),
            # Event API Product Versions — CRUD + state + publish
            RouteMap(pattern=r"^/api/v2/architecture/eventApiProductVersions(/\{versionId\})?$", mcp_type=MCPType.TOOL),
            RouteMap(pattern=r"^/api/v2/architecture/eventApiProductVersions/\{versionId\}/state$", mcp_type=MCPType.TOOL),
            RouteMap(pattern=r"^/api/v2/architecture/eventApiProductVersions/\{versionId\}/publish$", mcp_type=MCPType.TOOL),
            # Consumers — list applications consuming from Event APIs
            RouteMap(pattern=r"^/api/v2/architecture/consumers(/\{id\})?$", mcp_type=MCPType.TOOL),
            # Catch-all: exclude everything else not explicitly listed above
